...
class MoEBlock(nn.Module):
    def __init__(self, embedding_size, output):
        ...
        self.moe1 = nn.Parameter(torch.ones(embedding_size, output), requires_grad=True)
        self.moe2 = nn.Parameter(torch.ones(embedding_size, output), requires_grad=True)
        self.relu = nn.ReLU(inplace=True)
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, w_base=16, embedding_size=64):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.bn1 = nn.BatchNorm2d(w_base)
        self.layer1 = self._make_layer(block, w_base, num_blocks[0], stride=1, moe=False)
        self.layer2 = self._make_layer(block, w_base*2, num_blocks[1], stride=2, moe=False)
    ...
    def forward(self, x):
        ...
        sss=[]
        for i in e:
            # print(np.squeeze(i.detach().cpu().numpy()))
            sss.append(np.squeeze(i.detach().cpu().numpy()))

        import pandas as pd
        ss=pd.DataFrame(sss)
        ss.to_csv("./show/wide_bottom_01.csv")
        exit()
